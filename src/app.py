...
# DEBUG_MODE: Enables Flask debug mode for development
DEBUG_MODE = os.environ.get('DEBUG_MODE', 'False').lower() == 'true'
# RUN_GLOBALLY: If True, listens on 0.0.0.0, otherwise 127.0.0.1
RUN_GLOBALLY = os.environ.get('RUN_GLOBALLY', 'True').lower() == 'true'
# PORT_HTTP_SERVER: The port the Flask server will listen on
PORT_HTTP_SERVER = int(os.environ.get('PORT_HTTP_SERVER', 5000))
# IS_DOCKER: Set in Dockerfile to override host to 0.0.0.0
...
def get_live_delays():
    '''Reads the JSON file generated by collector.py'''
...
def departures():
    ...
    # Load delays from the background collector
    ...
    for target_date, is_today in [(yesterday, False), (today, True)]:
        ...
        for row in cursor.fetchall():
            delay_min = live_delays.get(str(row['block_id']))
            has_rt = delay_min is not None
            delay_min = int(delay_min) if has_rt else 0
